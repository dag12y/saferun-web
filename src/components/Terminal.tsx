  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-xs mono mx-auto" style={{ color: 'var(--terminal-dim)' }}>
          zsh — saferun
        </span>
      </div>
      ...
      <div className="p-5 font-mono text-sm leading-6 overflow-x-auto">
        {lines.map((line, i) => (
          <div key={i} style={{ color: line.type === 'blank' ? 'transparent' : (colorMap[line.type] || '#e2e8f0'), minHeight: '1.5rem' }}>
            {line.type === 'prompt' && (
              <span>
            ...
            {line.type === 'header' && (
              <span style={{ fontWeight: 600, color: '#f0f0f4' }}>{line.text}</span>
            )}
