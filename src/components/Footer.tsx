      ...
      <div className="max-w-6xl mx-auto px-5 py-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <span className="font-semibold text-sm" style={{ color: 'var(--fg)' }}>SafeRun</span>
          </div>
          <p className="text-sm" style={{ color: 'var(--fg3)' }}>Secure package installation sandbox</p>
          <p className="text-xs mt-2" style={{ color: 'var(--fg3)' }}>© {year} SafeRun. MIT License.</p>
        </div>
        ...
        <nav className="flex flex-wrap gap-x-6 gap-y-2">
          {[
            ...
            { label: 'License', href: 'https://github.com/dag12y/saferun/blob/main/LICENSE' },
          ].map(item => (
            item.href ? (
              <a
                key={item.label}
