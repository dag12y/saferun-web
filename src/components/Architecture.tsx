const steps = [
  ...
  { label: 'Docker Sandbox', sub: 'Disposable isolated container', icon: '⬡', accent: true },
  { label: 'Static Analysis', sub: 'Scripts, metadata, structure', icon: '⊡' },
  { label: 'Behavior Analysis', sub: 'Filesystem · Network · Processes', icon: '⊠' },
  { label: 'Risk Engine', sub: 'Scoring and classification', icon: '◈' },
  { label: 'Policy Decision', sub: 'ALLOW · CONFIRM · BLOCK', icon: '⊕', accent: true },
    ...
    <div className="flex flex-col items-center gap-0 w-full max-w-xs mx-auto select-none">
      {steps.map((step, i) => (
        <div key={i} className="flex flex-col items-center w-full">
          ...
          >
            <span className="mono text-base w-6 text-center flex-shrink-0" style={{ color: 'var(--accent)' }}>
              {step.icon}
            </span>
            <div>
              <div className="text-sm font-semibold" style={{ color: 'var(--fg)' }}>{step.label}</div>
              <div className="text-xs mono mt-0.5" style={{ color: 'var(--fg3)' }}>{step.sub}</div>
            </div>
          </div>
          {i < steps.length - 1 && (
