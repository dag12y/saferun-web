...
function Section({
  ...
  return (
    <div
      id={id}
      className="pt-2 pb-12 border-b last:border-0"
...
function H3({ children }: { children: React.ReactNode }) {
  return (
    <h3
      className="font-semibold text-base mb-2 mt-6"
      style={{ color: "var(--fg)" }}
    >
      {children}
    </h3>
        ...
        <thead>
          ...
          >
            {["Variable", "Purpose", "Default"].map((h) => (
              <th
                key={h}
                className="text-left px-4 py-2.5 font-medium mono text-xs"
                style={{ color: "var(--fg3)" }}
              >
                {h}
              </th>
        ...
        <tbody>
          {rows.map(([name, purpose, def], i) => (
            <tr
              key={name}
              style={{
                background: i % 2 ? "var(--card)" : "transparent",
                borderBottom: "1px solid var(--border)",
              }}
            >
              <td
  ...
  return (
    <div className="max-w-6xl mx-auto px-5 py-10 flex gap-10">
        ...
        <div className="sticky top-20 overflow-y-auto max-h-[calc(100vh-6rem)] scrollbar-thin pr-2">
          {NAV.map((group) => (
            <div key={group.section} className="mb-6">
              <p
                className="text-xs mono font-semibold mb-2 px-2"
                style={{ color: "var(--fg3)" }}
              >
          ...
          >
            {NAV.map((group) => (
              <div key={group.section} className="mb-4">
                <p
                  className="text-xs mono font-semibold mb-1.5 px-2"
                  style={{ color: "var(--fg3)" }}
                >
                  {group.section.toUpperCase()}
                </p>
                {group.items.map((item) => (
                  <button
                    ...
                    style={{
                      color:
                        active === item.id ? "var(--accent)" : "var(--fg2)",
                      background:
                        active === item.id
                          ? "var(--accent-dim)"
                          : "transparent",
      ...
      <div className="flex-1 min-w-0 max-w-3xl">
        {/* breadcrumb */}
        <p className="text-xs mono mb-8" style={{ color: "var(--fg3)" }}>
          SafeRun / Documentation
        </p>

        {/* Introduction */}
        <Section id="what-is-saferun" title="What is SafeRun?">
          <P>
            SafeRun is a command-line tool written in Go that intercepts npm
            package installation and runs the package inside a disposable Docker
            container before allowing it onto your host system. It monitors
            behavior during sandbox execution and produces a security report
        ...
        <Section id="why-saferun" title="Why SafeRun?">
          <P>
            npm packages execute arbitrary code during installation through
            lifecycle scripts (preinstall, install, postinstall). Malicious
            packages have used these hooks to exfiltrate credentials, establish
            persistence, or tamper with host files.
          </P>
          <P>
            SafeRun reduces the attack surface by running the full installation
            — including lifecycle scripts — inside an isolated Docker container
            where behavior can be observed before any host-side changes are
            made.
          </P>
          <Callout type="note">
            SafeRun reduces risk. It does not eliminate it. Sufficiently
            sophisticated packages may detect sandbox environments or exploit
            vulnerabilities in the analysis layer.
          </Callout>
        </Section>
        ...
        <Section id="how-it-works" title="How it works">
          ...
          <ol
            className="list-decimal list-inside space-y-2 mb-4 text-sm"
            style={{ color: "var(--fg2)" }}
          >
            <li>Downloads the package tarball from the npm registry</li>
            <li>Verifies the SHA-256 checksum of the downloaded archive</li>
            <li>
              Starts a disposable Docker container with the SafeRun sandbox
              image
            </li>
            ...
            <li>
              Asks for confirmation (if required by policy) before installing to
              the host
            </li>
            <li>
              Appends an audit event to{" "}
        ...
        <Section id="architecture" title="Architecture">
          <P>
            SafeRun is organized into internal Go packages covering CLI parsing,
            sandbox management, behavior analysis, risk scoring, and reporting.
          </P>
          <CodeBlock
            lang="text"
            showCopy={false}
...
sandbox/images/node/         — Sandbox Docker image (Node.js + npm)`}
        ...
        <Section id="security-model" title="Security model">
          <P>
            SafeRun's security model is based on sandbox-first installation: no
            package code runs on the host before analysis. The Docker container
            is disposed of after analysis regardless of outcome.
          </P>
          <P>
            SafeRun does not query external threat intelligence services,
            package reputation databases, or vulnerability feeds. All analysis
            is local and behavioral.
          </P>
        </Section>
        ...
        <Section id="install-linux" title="Installation — Linux">
          ...
          <CodeBlock
            lang="bash"
            code={`curl -fsSL https://www.saferun.tech/install.sh | sh`}
          />
          <P>
            ...
            and verifies the SHA-256 checksum before installation. No root
            access is required.
          </P>
...
install -m 0755 saferun-linux-amd64 ~/.local/bin/saferun`}
          ...
          <P>
            ...
            SafeRun uses Docker to create and manage sandbox containers.
          </P>
        </Section>
        ...
        <Section id="install-macos" title="Installation — macOS">
          ...
          />
          <P>
            Binaries are available for both amd64 (Intel) and arm64 (Apple
            Silicon). The installer selects the correct architecture
            automatically.
          </P>
          <Callout type="warn">
            Docker Desktop for Mac must be running before using SafeRun.
          </Callout>
        </Section>
        ...
        <Section id="install-windows" title="Installation — Windows">
          ...
          <CodeBlock
            lang="powershell"
            code={`Invoke-Expression (Invoke-WebRequest -Uri https://www.saferun.tech/install.ps1 -UseBasicParsing).Content`}
          />
          <P>
            The installer places SafeRun in{" "}
            <code
              className="mono text-xs px-1 py-0.5 rounded"
              style={{ background: "var(--card2)", color: "var(--accent)" }}
            >
              %LOCALAPPDATA%\SafeRun\bin
            </code>{" "}
            and adds it to the user PATH. No administrator rights are required.
        ...
        <Section id="quick-start" title="Quick start">
          <P>
            After installation, verify SafeRun works and Docker is accessible:
          </P>
          <CodeBlock lang="bash" code={`saferun setup`} />
          <P>Then install a package:</P>
          <CodeBlock lang="bash" code={`saferun npm install lodash`} />
          <P>
            SafeRun will sandbox the installation, show you the security report,
            and ask for confirmation before installing to the host.
          </P>
        </Section>
...
saferun --version          Print the SafeRun version`}
        ...
        <Section id="saferun-setup" title="saferun setup">
          <P>
            Verifies that Docker is running and that the SafeRun sandbox image
            is available. Run this once after installation to confirm the
        ...
        <Section id="saferun-npm-install" title="saferun npm install">
          ...
          <CodeBlock
            lang="bash"
            code={`saferun npm install <package>[@version]`}
          />
          <P>Example:</P>
          <CodeBlock
            lang="bash"
            code={`saferun npm install express
saferun npm install lodash@4.17.21`}
        ...
        <Section id="saferun-version" title="saferun --version">
          <P>
            Prints the SafeRun version. Release builds report the injected Git
            tag. Local development builds report{" "}
            <code
              className="mono text-xs px-1 py-0.5 rounded"
              style={{ background: "var(--card2)", color: "var(--fg3)" }}
            >
              SafeRun dev
            </code>
            .
        ...
        <Section id="sandbox" title="Sandbox">
          <P>
            SafeRun runs each package installation inside a disposable Docker
            container built from the SafeRun sandbox image. The image contains
            Node.js and npm, isolated from the host network and filesystem by
            default Docker security settings.
          </P>
          <P>
            The container is created fresh for each SafeRun invocation and
            destroyed after analysis completes, regardless of outcome.
          </P>
          <Callout type="note">
            Full sandbox hardening (seccomp, AppArmor profiles, read-only
            mounts, network namespace isolation) is planned but not yet
        ...
        <Section id="behavior-analysis" title="Behavior analysis">
          ...
          >
            <li>
              ...
              — files created, modified, or deleted outside expected npm
              directories
            </li>
            <li>
              <strong style={{ color: "var(--fg)" }}>Network activity</strong> —
              outbound connections made during package installation
            </li>
            <li>
              <strong style={{ color: "var(--fg)" }}>Process activity</strong> —
              subprocesses spawned by lifecycle scripts
            </li>
          </ul>
          <Callout type="note">
            Behavior monitoring is under active development. Not all monitors
        ...
        <Section id="risk-scoring" title="Risk scoring">
          <P>
            After sandbox execution, SafeRun's risk engine assigns a numeric
            score and classifies the result into one of four tiers:
          </P>
          <EnvTable
            rows={[
        ...
        <Section id="security-policy" title="Security policy">
          ...
          >
            <li>
              <strong style={{ color: "#10d078" }}>ALLOW</strong> — proceed with
              host installation automatically
            </li>
            <li>
              <strong style={{ color: "#fbbf24" }}>CONFIRM</strong> — display
              report and ask user to approve or reject
            </li>
            <li>
              <strong style={{ color: "#f87171" }}>BLOCK</strong> — reject
        ...
        <Section id="fail-closed" title="Fail-closed behavior">
          <P>
            If the Docker daemon is not running, the sandbox image is
            unavailable, or analysis fails for any other reason, SafeRun fails
            closed: installation does not proceed. This ensures that bypassing
            the sandbox (accidentally or deliberately) is not the path of least
            resistance.
          </P>
        </Section>
        ...
        <Section id="env-vars" title="Environment variables">
          <EnvTable
            rows={[
              [
                "SAFERUN_VERSION",
                "Release tag to download during installation",
                "current release",
              ],
              [
                "SAFERUN_RELEASE_BASE_URL",
        ...
        <Section id="install-dir" title="Installation directory">
          <P>
            On Linux and macOS, SafeRun installs to{" "}
            <code
              className="mono text-xs px-1 py-0.5 rounded"
              style={{ background: "var(--card2)", color: "var(--accent)" }}
            >
          ...
          <P>
            On Windows, SafeRun installs to{" "}
            <code
              className="mono text-xs px-1 py-0.5 rounded"
              style={{ background: "var(--card2)", color: "var(--accent)" }}
            >
        ...
        <Section id="proxy" title="Proxy configuration">
          <P>
            SafeRun respects standard HTTP proxy environment variables:{" "}
            <code
              className="mono text-xs px-1 py-0.5 rounded"
              style={{ background: "var(--card2)", color: "var(--accent)" }}
            >
        ...
        <Section id="audit-logs" title="Audit logs">
          <P>
            Every SafeRun operation appends a structured JSON event to{" "}
            <code
              className="mono text-xs px-1 py-0.5 rounded"
              style={{ background: "var(--card2)", color: "var(--accent)" }}
            >
            ...
            . The log is append-only and human-readable (one JSON object per
            line).
          </P>
  ...
  "installed": true
        ...
        <Section id="ts-docker" title="Docker not running">
          <P>
            SafeRun requires Docker. If you see an error about Docker not being
            accessible:
          </P>
          <CodeBlock
            lang="bash"
            code={`# Check Docker daemon status
docker info
...
sudo systemctl start docker`}
          />
          <P>
            On macOS and Windows, start Docker Desktop before running SafeRun.
          </P>
        </Section>

        <Section id="ts-network" title="Network / proxy issues">
          <P>
            If SafeRun cannot download packages or the sandbox image, verify
...
saferun npm install lodash`}
        ...
        <Section id="ts-permissions" title="Permission problems">
          <P>
            On Linux, if SafeRun cannot connect to Docker, add your user to the
            docker group:
          </P>
          <CodeBlock
            lang="bash"
